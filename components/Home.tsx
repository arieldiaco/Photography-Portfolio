...
export const Home: React.FC<HomeProps> = ({ photos, onPhotoChange }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Ariel Diacovetzky Photo Journal',
          text: currentPhoto.description || 'Check out this photo by Ariel Diacovetzky',
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-white transition-colors duration-500 overflow-hidden flex flex-col items-center justify-center p-4">
      ...
      >
        <div className="w-full shadow-sm bg-zinc-50 border border-zinc-100 relative group">
          <img 
          ...
          <button 
            onClick={handleShare}
            className="absolute bottom-4 right-4 p-3 bg-white/80 backdrop-blur-md rounded-full text-zinc-900 shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-95"
